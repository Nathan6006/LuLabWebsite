...
Every grey box with a dashed border is a placeholder where a photo should go.
Each one is already the correct shape and has a caption saying what belongs
there. No photos have been added yet — see `TODO.md` for the full list.
...
Fonts (Source Serif 4, Public Sans) are downloaded at build time by Astro's font
pipeline and self-hosted, with metric-matched fallbacks so no text reflows on
load. Colours are defined once as design tokens in `src/styles/global.css` and
taken from the current CWRU brand palette.
